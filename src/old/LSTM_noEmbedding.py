...
from keras.layers import Input, LSTM, TimeDistributed, Dense, Attention, Concatenate
...
from misc import predict_sequence_LSTMnoEmbedding, load_X, load_y, index_representation
...
### LOAD DATA ----------------------------------------------------------------------------
data_folder_train = f'{path_input}training_data2/'
...
### TRAINING MODEL --------------------------------------------------------------------------------
# Parameters
LEN_INPUT_SEQUENCE = X_train.shape[1] # Number of fequency/wavelength samples
...
# Decoder ---
dec_inputs = Input(shape=(None, SOIL_VOCAB_SIZE))

#Layer 1
dec_lstm1 = LSTM(LATENT_DIM*2, return_sequences=True, return_state=True, dropout=0, recurrent_dropout=0)
dec_lstm1_outputs, _, _ = dec_lstm1(dec_inputs, initial_state=enc_states1)
...
# Name model
model_name = f'{path_models}/[{datetime.now().strftime("%Y%m%d%H%M")}]_LSTM_noEmbedding'
...
decoder_lstm1_outputs, state_h1, state_c1 = dec_lstm1(dec_inputs, initial_state=decoder_states_inputs[:2])
...
for i_sample in range(N_samples_test):
    input_seq = X_test[i_sample, ...].reshape(1, X_test.shape[1], X_test.shape[2])
